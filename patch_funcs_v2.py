...
def replace_func(lines_list, func_name, new_code):
    start_line = -1
    ...
    end_line = -1
    started = False
    for i in range(start_line, len(lines_list)):
        brace_count += lines_list[i].count('{')
        brace_count -= lines_list[i].count('}')
        if '{' in lines_list[i]: started = True
        if started and brace_count == 0:
            end_line = i
            break
    if end_line == -1: return False
    
    lines_list[start_line:end_line+1] = [new_code + '\n']
    return True
...
if replace_func(lines, 'renderStatRow', new_stat_row) and replace_func(lines, 'renderPercentRow', new_percent_row):
