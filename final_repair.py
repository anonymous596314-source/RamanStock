...
def repair():
    ...
    for i, line in enumerate(lines):
        if '// === Indicators Encyclopedia' in line:
            truncate_idx = i
            break
    
    # 如果沒找到，尋找最後一個正常的函數 (renderValuationRiverMap)
    if truncate_idx == -1:
        for i in range(len(lines)-1, -1, -1):
            if 'function renderValuationRiverMap' in lines[i]:
                # 尋找結尾大括號
                brace_count = 0
                for j in range(i, len(lines)):
                    ...
                    if j > i and brace_count == 0:
                        truncate_idx = j + 1
                        break
                if truncate_idx != -1: break

    if truncate_idx == -1:
        print('Could not find truncation point')
        return

    print(f'Truncating at line {truncate_idx}')
